...
void UndirectedWeightedGraph::kruskalsAlgorithm()
{
	...
	for(int u=0; u<nVertices; u++)
	{
		for(int v=u; v<nVertices; v++)
		{
			if(isAdjacent(u,v))
			{
				edgeQueue.push(Edge(u,v,adj[u][v]));
			}
		}
	}
	...
	while(!edgeQueue.empty() && count < nVertices-1)
	{
		...
		{
			count++;
			treeEdges[count].u = edge.u;
			treeEdges[count].v = edge.v;
...
void UndirectedWeightedGraph::minimumSpanningTree()
{
	...
	{
		cout << "Edge - (" << vertexList[treeEdges[i].u]->name << "-" << vertexList[treeEdges[i].v]->name << ")\n";
